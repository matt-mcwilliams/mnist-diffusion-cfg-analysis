...
if __name__ == '__main__':
  ...
  for d in range(0,10):
    labels, samples = generate_samples(model, num_generations=3, digit=d, guidance_scale=3, random_seed=1, device=device)
    for i, sample in enumerate(samples):
      img = sample.squeeze().cpu().detach().numpy()
      plt.imshow(img, cmap='gray')
      plt.axis('off')
      plt.savefig(f'{args.sample_path}/d-{d}-{i+1}.png', bbox_inches='tight')
      plt.close()
